...
// SNAF
$current_listing_style = PRODUCT_LISTING_LAYOUT_STYLE;
...
{
    $current_listing_style = $_SESSION['PRODUCT_LISTING_LAYOUT_STYLE'];
}
$row = 0;
$col = 0;
...
$max_results = ($current_listing_style=='columns' && PRODUCT_LISTING_COLUMNS_PER_ROW>0) ? (PRODUCT_LISTING_COLUMNS_PER_ROW * (int)(MAX_DISPLAY_PRODUCTS_LISTING/PRODUCT_LISTING_COLUMNS_PER_ROW)) : MAX_DISPLAY_PRODUCTS_LISTING;

$show_submit = zen_run_normal();

if($use_product_listing)
{
$listing_split = $snaf_listing_split;
}else{
...
if ($current_listing_style == 'rows') {		// For Column Layout (Grid Layout) add on module
...
if ($listing_split->number_of_rows > 0) {
  ...
  while (!$listing->EOF) {
    ...
    if ($current_listing_style == 'rows') { // Used in Column Layout (Grid Layout) Add on module
    $rows++;
...
//        $lc_align = '';
        ...
        break;
        case 'PRODUCT_LIST_PRICE':
        $lc_price = '<div class="prodListingPrice">' . zen_get_products_display_price($listing->fields['products_id']) . '</div>';
//        $lc_align = 'right';
        $lc_text =  $lc_price;
        ...
        if (zen_has_product_attributes($listing->fields['products_id']) or PRODUCT_LIST_PRICE_BUY_NOW == '0') {
          ...
          if (PRODUCT_LISTING_MULTIPLE_ADD_TO_CART != 0) {
            ...
            if ($listing->fields['products_qty_box_status'] == 0) {
              ...
              $lc_button = '<label class="listingLabel">'. TEXT_PRODUCT_LISTING_MULTIPLE_ADD_TO_CART . "</label><input type=\"text\" name=\"products_id[" . $listing->fields['products_id'] . "]\" value=\"0\" size=\"4\" />";
            }
          } else {
...
//        $lc_align = 'center';
        if ($listing->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) {
          ...
          if (isset($_GET['manufacturers_id'])) {
            $lc_text = '<a class="product_listing_image_link" href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . (($_GET['manufacturers_id'] > 0 and $_GET['filter_id']) > 0 ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? zen_get_generated_category_path_rev($_GET['cPath']) : zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $listing->fields['products_image'], $listing->fields['products_name'], IMAGE_PRODUCT_LISTING_WIDTH, IMAGE_PRODUCT_LISTING_HEIGHT, 'class="listingProductImage"') . '</a>';
          } else {
            $lc_text = '<a class="product_listing_image_link" href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . (($_GET['manufacturers_id'] > 0 and $_GET['filter_id']) > 0 ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? zen_get_generated_category_path_rev($_GET['cPath']) : zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $listing->fields['products_image'], $listing->fields['products_name'], IMAGE_PRODUCT_LISTING_WIDTH, IMAGE_PRODUCT_LISTING_HEIGHT, 'class="listingProductImage"') . '</a>';
          }
        }
...
//===========  bof SNAF (add style switcher)

if(USE_STYLE_CHANGER_FOR_PRODUCT_LISTING == 'True')
{
require($template->get_template_dir('/tpl_snaf_style_changer.php',DIR_WS_TEMPLATE, $current_page_base,'templates'). '/tpl_snaf_style_changer.php');
}
//===========  eof SNAF (add style switcher)

// removed conditional for version 1.2 - forum 1116858
//if ($current_listing_style == 'columns') {
